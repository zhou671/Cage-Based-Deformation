...
double custom_determinent(Matrix3d M){
    double a1 = M(0, 0) * M(1, 1) * M(2, 2);
    double a2 = M(0, 1) * M(1, 2) * M(2, 0);
    double a3 = M(1, 0) * M(2, 1) * M(0, 2);
    double b1 = M(0, 2) * M(1, 1) * M(2, 0);
    double b2 = M(0, 1) * M(1, 0) * M(2, 2);
    double b3 = M(1, 2) * M(2, 1) * M(0, 0);
...
void MeanValueCoordController::ComputeMVWeights() {
    ...
    for (int k = 0; k < num_vertices_mesh; k++){
        ...
        for (int f = 0; f < num_faces_cage; f++){
            ...
            double sIs[3] = {0.,0.,0.};
            double h = 0.;
            Matrix3d u123 = Matrix3d::Zero(3,3);
            for (int i = 0; i < 3; i++){
                int iMinus = (i-1) % 3;
            ...
            //double signDet = u123.determinent();
            double signDet = custom_determinent(u123);
            if (signDet == 0){
                cout << "u123 = " << endl;
